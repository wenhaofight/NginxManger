...
public class TestUtils {
    ...
    public static void assertParam(NgxEntry entry, String name, String... values) {
        Assertions.assertTrue(entry instanceof NgxParam);
        Assertions.assertEquals(((NgxParam) entry).getName(), name);

        Iterator<String> it = ((NgxParam) entry).getValues().iterator();
        for (String value : values) {
            Assertions.assertEquals(it.next(), value);
        }

    }

    public static NgxBlock assertBlock(NgxEntry entry, String name, String... values) {
        Assertions.assertTrue(entry instanceof NgxBlock);
        Assertions.assertEquals(((NgxBlock) entry).getName(), name);

        Assertions.assertEquals(values.length, ((NgxBlock) entry).getValues().size());
    ...
    public static NgxIfBlock assertIfBlock(NgxEntry entry, String name, String... values) {
        Assertions.assertTrue(entry instanceof NgxBlock);
        Assertions.assertEquals(((NgxIfBlock) entry).getName(), name);
    ...
    public static NgxComment assertComment(NgxEntry entry, @SuppressWarnings("SameParameterValue") String value) {
        Assertions.assertTrue(entry instanceof NgxComment);
        Assertions.assertEquals(((NgxComment) entry).getValue(), value);
